...
As described in footnote 2 on page 3 of Carpenter (2107), RStan takes code written in Stan, translates it into C++, compiles it, runs it, and saves the output to an object in R. This means that installation is a lot trickier than for most R libraries.
...
All variables in Stan are static, meaning that they can't change from integer to real or from vector to matrix in the middle of the program. You need to be careful when you mix integer and real values. Usually it works, but there are a few exceptions.
